...
class CpucSpider(scrapy.Spider):
        ...
        for row in table_rows:
            ...
            if len(proceeding_set) > 0:
                next_page = 'https://apps.cpuc.ca.gov/apex/f?p=401:56:6062906969229::NO:RP,57,RIR' \
                            ':P5_PROCEEDING_SELECT:{}'.format(proceeding_set.pop())
                yield response.follow(next_page,
                                      callback=self.parse_proceeding_number,
                                      meta={'docket_list': dockets})
        formdata = {
    ...
    def parse_proceeding_number(self, response):
        ...
        yield scrapy.Request(
            ...
            dont_filter=True,
            meta={'item': proceeding_details, 'dont_merge_cookies': True, 'docket': response.meta['dockets']}
        )
