""" This is the scraper for vermont website dockets """
...
class VermontSpider(scrapy.Spider):
    """ This is the class of web spider of vermont website dockets """
    ...
    def search_case(self, response):
        ...
        formdata = {
            'data(181445)': search_start_date,
            'data(181445_right)': search_end_date,
        }
    ...
    def parse_proceeding_numbers(self, response):
        """In this method, all docket numbers are turned into
            a list and send for getting infromation. """
    ...
    def parse_proceeding_details(self, response):
        ...
        title = self.removed_garbage_from_data(
    ...
    def parse_filing_parties(self, response):
        ...
        filing_parties = response.xpath("//tr[starts-with(@id, 'tree.branch.')]"
    ...
    def parse_prefiled_testimony_exhibits_documents(self, response):
        """ Parse prefiled testimony exhibts documents """
    ...
    def parse_filings(self, filings, response):
        ...
        for filing in filings_data:
            ...
            if self.get_column_count(filing) == 2:
                ...
                if not filing_loader.get_output_value('filled_on'):
                ...
                filing_loader.replace_value('description',
                                            filing.xpath("td[3]/text()").get().strip())
                ...
                filing_parties = self.removed_garbage_from_data(
                    filing.xpath("td[5]/text()").getall())
    ...
    @staticmethod
    def get_date(data):
        """ if date in string then return """

        return re.search(r'(\d+/\d+/\d+)', data)

    @staticmethod
    def parse_document_details(document_url, docket_loader):
        """ make data of a document and return """
    ...
    @staticmethod
    def get_filing(response, filing):
        """ return filing data """

        filings_data = response.xpath("//table[starts-with(@id, 'form.treeTable')]"
                                      "/thead/tr[2]/th[5]/text()").get().strip()
    ...
    def get_column_count(filing):
        """ return how much column in each row """
    ...
    def removed_garbage_from_data(data_list):
        """ remove garabage of data and return string """

        return [data.strip() for data in data_list if data.strip() and data.strip() != ':']

    def handle_legacy_request(self, response):
        """  handle legacy case parse filed on and replace url for further data and return """
    ...
    def remove_garbage_from_string(data):
        """ remove garabage of string and return """
