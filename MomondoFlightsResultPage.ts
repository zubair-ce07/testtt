...
export class MomondoFlightsResultsPage {
    ...
    originAirportOfLegOne: ElementFinder = element(by.css("input[id$=origin0-airport]"));
    originAirportOfLegTwo: ElementFinder = element(by.css("input[id$=origin1-airport]"));
    destinationAirportOfLegOne: ElementFinder = element(by.css("input[id$=destination0-airport]"));
    destinationAirportOfLegTwo: ElementFinder = element(by.css("input[id$=destination1-airport]"));
    cabinsSelectBoxOfLegOne: ElementFinder = element(by.css("select[id$=cabin_type0-select]"));
    cabinsSelectBoxStatusOfLegOne: ElementFinder = element(by.css("span[id$=cabin_type0-status]"));
    cabinsSelectBoxStatusOfLegTwo: ElementFinder = element(by.css("span[id$=cabin_type1-status]"));    
    businessCabinOptionInLegOne: ElementFinder = this.cabinsSelectBoxStatusOfLegOne.element(by.css("option[title=Business]"));
    cabinsSelectBoxOfLegTwo: ElementFinder = element(by.css("div[id$=cabin_type1]"));
    businessCabinOptionInLegTwo: ElementFinder = this.cabinsSelectBoxStatusOfLegTwo.element(by.css("option[title=Business]"));
    dateBoxOfLegOne: ElementFinder = this.legOneOfMultipleCities.element(by.css("div[id$=depart_date0-input]"));
    dateBoxOfLegTwo: ElementFinder = this.legTwoOfmultipleCities.element(by.css("div[id$=depart_date1-input]"));
    searchForm: ElementFinder = element(by.css("form[name=searchform]"));
    travelerDropDown: ElementFinder = element(by.css("div[class*='travelersBlock']"));
    adultTravelers: ElementFinder = element(by.css("input[id$=adults-input]"));
    youthTravelers: ElementFinder = element(by.css("input[id$=youth-input]"));
    childTravelers: ElementFinder = element(by.css("input[id$=child-input]"));
    seatInfantTravelers: ElementFinder = element(by.css("input[id$=seatInfant-input]"));
    lapInfantTravelers: ElementFinder = element(by.css("input[id$=lapInfant-input]"));
    ...
    async defaultTravelersSelected(): Promise<boolean> {
        ...
        return (adultTravelers === "1" && youthTravelers === "0"
    ...
    async resultsContainNewTimeRangeForSecondLeg(): Promise<boolean> {
		...
		return this.flightResults.then(async function(results) {
			for(let result of results) {
				let timeInResults: string = await result.element(by.css("div[id$=info-leg-1] .time.depart")).getText();
