...
class BookScraperSpider(scrapy.Spider):
    ...
    def parse_book_details(self, response):
        ...
        book["title"] = response.css('div.page_inner > ul.breadcrumb li.active::text').extract_first()
        book["category"] = response.css('div.page_inner > ul.breadcrumb li>a::text').extract()[2]
        book["description"] = response.css('div#product_description + p::text').extract()
        book["product_information"] = dictionary
        book["rating"] = response.css('div.product_main > p.star-rating::attr(class)').extract_first().split()[1]
        book["price"] = response.css('div.product_main p.price_color::text').extract()
