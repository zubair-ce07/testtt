...
user_data = {'username': 'zainab_student', 'first_name': 'Zainab',
...
class StudentReadTest(APITestCase):
    def setUp(self):
        other_user_data = {'username': 'newstudent', 'first_name': 'Zainab',
                           'last_name': 'Amir', 'email': 'zainab.amir@arbisoft.com'}
        user, self.student = create_student(user_data)
        other_user, self.other_student = create_student(other_user_data)
    ...
    def test_can_read_own_details(self):
        response = self.client.get(reverse('student-detail', args=[self.student.id]))
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_can_read_others_details(self):
        response = self.client.get(reverse('student-detail', args=[self.other_student.id]))
...
#         self.client.login(username='john', password='johnpassword')
#         user, self.student = create_student(user_data)
#         self.data = serializers.StudentSerializer(self.student).data
#         self.data.update({'first_name': 'Changed'})
#
#     def test_can_update_user(self):
#         response = self.client.put(reverse('user-detail', args=[self.student.id]), self.data)
#         self.assertEqual(response.status_code, status.HTTP_200_OK)
