...
class NovoEdSpider(scrapy.Spider):
    ...
    urls = ['https://api.novoed.com/{0}/learner_participation_details_in_course.json?api_key={1}&api_secret={2}', ]
    ...
    def parse_page_results(self, response):
        ...
        links = []
        course_name = response.meta["course_id"]
        data_file = response.body
        data = json.loads(data_file)
        for learner in data["result"]["learners"]:
            key = course_name + "_" + str(learner["id"]) + "_"
            for exercise in learner["exercises"]:
                key_level1 = key + str(exercise["id"]) + "_"
                if "details" in exercise.keys():
                    for details in exercise["details"]:
                        key_level2 = key_level1 + str(details["report_section_template_id"])
                        if "response" in details.keys():
                            if details["response"].startswith('http'):
                                links.append({
        ...
        if links:
            for link in links:
                yield Request(link["url"], callback=self.parse_file, meta=link, dont_filter=True, priority=1)
    ...
    def parse_file(self, response):
        ...
        if response.status == 200:
            ...
            filename = response.meta["key"]
            extension = mimetypes.guess_extension(response.headers["Content-Type"])
            filename += extension
            with open(dir_name + filename, "w") as datafile:
                datafile.write(response.body)
            print("saved.", filename)
        else:
            print("error")
            self.error_count += 1
            self.error_files.append(deepcopy(response.meta))
            if self.error_count % 10 == 0:
                with open("error_file_urls.json", "w") as error_file:
                    error_file.write(str(self.error_files))
