...
class SearsSpider(scrapy.Spider):
    ...
    def parse_product_item(self, response):
        ...
        product_id = re.search(r'p-\w+', response.request.url).group()[2:]
        url = "https://www.sears.com/content/pdp/config/products/v1/products/" \
              + product_id + "?site=sears"
        yield Request(url, callback=self.parse_product_info, meta={"product_id" : product_id})


    def parse_product_info(self, response):
        json_response = json.loads(response.body_as_unicode())
        product_id = str(json_response["data"]["offerranking"]["id"])

        self.set_breadcrumbs(json_response)
        self.item["store_keeping_unit"] = json_response["data"]["product"]["id"]
        self.item["title"] = json_response["data"]["product"]["seo"]["title"]
        self.set_description(json_response)
        self.set_brand(json_response)

        self.item['variations'] = []
        if json_response["data"].get("attributes", False): # If variations exist
            ...
            url = "https://www.sears.com/content/pdp/products/pricing/v2/get/price/display" \
                  "/json?offer=" + product_id + "&priceMatch=Y&memberType=G&site=SEARS"
            yield Request(url,
    ...
    def set_description(self, json_data):
        self.item["description"] = []
        descriptions = json_data["data"]["product"]["desc"]
        if descriptions:
            for description in descriptions:
                self.item["description"].append(description["val"])


    def set_breadcrumbs(self, json_data):
        breadcrumbs = json_data["data"]["productmapping"]["primaryWebPath"]
        if breadcrumbs:
            self.item["breadcrumbs"] = []
            for breadcrumb in breadcrumbs:
                self.item["breadcrumbs"].append(breadcrumb["name"])

    def set_brand(self, json_data):
        try:
            self.item["brand"] = json_data["data"]["product"]["brand"]["name"]
    ...
    def process_and_add_variation(self, variation, should_yield):  # Parse Variation Item
        """
        The json data contains a new variation for each size item.
    ...
    def get_meta_data_from_variation(self, variation, should_yield):
        return {
            "size" : self.get_size(variation) if variation is not None else "Self",
            "is_available" : variation["isAvailable"] if variation is not None else "None",
            "variation_name" : self.get_colour_name(variation) if variation is not None
            else "Self",
    ...
    def parse_size_item(self, response): #  Parse Size Item
        ...
        self.append_size_item_in_respective_variation(new_size_item,
                                                      response.meta["variation_name"])
    ...
    def get_price_discountedprice_isdiscounted(self, size_data):
        ...
        if detailed_size_data:
            ...
            savings = detailed_size_data[0]["savings"]["numeric"]
    ...
    def append_size_item_in_respective_variation(self, size_item, variation_name):
        ...
        for variation in self.item.get('variations', []):
            if variation['sizes'] is None:
                variation['sizes'] = []
            if variation['display_color_name'] == variation_name:
                variation['sizes'].append(size_item)
                didInsert = True
        if not didInsert:
            new_variation = items.VariationItem()
            new_variation['display_color_name'] = variation_name
            self.item["variations"].append("display_color_name")
            self.append_size_item_in_respective_variation(size_item, variation_name)
