...
class Origin {
  ...
  get() {
    return element(by.id(`${this.id}-origin`))
  }
    ...
    return multi.isPresent().then(async yes => {
      if (yes) {
        await multi.all(by.tagName('button'))
          ...
          .then(() => {
            const elm = element(by.id(`${this.id}-origin-airport`));
            browser.wait(ExpectedConditions.elementToBeClickable(elm));
  ...
  type(origin: string, clearPreviousInput = true) {
    const elm = element(by.id(`${this.id}-origin-airport`));
    ...
    return Promise.resolve(clearPreviousInput ? this.clear() : false)
      .then(async () => {
        ...
        browser.wait(ExpectedConditions.visibilityOf(elm));
        browser.wait(ExpectedConditions.elementToBeClickable(elm));
        return elm.sendKeys(origin);
      })
  }
  
  select(option: number) {
    // p_Ro-origin-airport-smartbox-dropdown
    browser.wait(ExpectedConditions.visibilityOf(element(by.id(`${this.id}-origin-airport-smarty-content`))));
    const elm = element.all(by.id(`${this.id}-origin-airport-smartbox-dropdown`))
  ...
  async getSearchResults(): Promise<Airport[]> {
    return element(by.id(`${this.id}-origin-airport-smartbox-dropdown`))
      .all(by.tagName('li'))
      .then(async elements => {
        const airports = [];
  ...
  private getMultiContainer() {
    return element(by.id(`${this.id}-origin-airport-display-multi-container`));
  }
}
...
class Destination {
  ...
  get() {
    return element(by.id(`${this.id}-destination`))
  }
  ...
  clear() {
    ...
    return multi.isPresent()
      .then(async yes => {
        const elm = multi.all(by.tagName('button'));
        
        if (yes) {
          const total = await elm.count();
          await multi.all(by.tagName('button'))
            .then(async () => {
              if (total > 0) {
                await elm.each(button => button.click());
                elm.sendKeys(Key.ESCAPE);
              }
            })
        }
  ...
  type(origin: string, clear = true) {
    const elm = element(by.id(`${this.id}-destination-airport`));
    
    return Promise.resolve(clear ? this.clear() : false)
      .then(async () => {
        browser.wait(ExpectedConditions.elementToBeClickable(this.get()));
        await this.get().click();
  ...
  select(option: number) {
    browser.wait(ExpectedConditions.visibilityOf(element(by.id(`${this.id}-destination-airport-smarty-content`))));
    const elm = element.all(by.id(`${this.id}-destination-airport-smartbox-dropdown`))
      .first().all(by.tagName('li')).get(option - 1);
  ...
  async getSearchResults(): Promise<Airport[]> {
    return element(by.id(`${this.id}-destination-airport-smartbox-dropdown`))
      .all(by.tagName('li'))
      .then(async elements => {
        const airports = [];
  ...
  getSelectedValue() {
    const multi = this.getMultiContainer();
    return multi.isPresent().then(isMultiContainer => {
      if (isMultiContainer) {
        const elm = multi.all(by.className('js-selection-display')).first();
        browser.wait(ExpectedConditions.visibilityOf(elm));
        return elm.getText()
  ...
  private getMultiContainer() {
    return element(by.id(`${this.id}-destination-airport-display-multi-container`));
  }
}
