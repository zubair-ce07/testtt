...
CLEANER_TEXT = [(r'\s{2,}\b|\"', '\''), (r':', '\':')]
GENDERS = ['Men', 'Women']
TEXT_TO_BE_REMOVED = ['null']
...
class UggCrawler(CrawlSpider):
    ...
    rules = (
        Rule(LinkExtractor(allow=r'/catalog/', restrict_css=('.btn-show-all',)),
    ...
    def parse_page(self, response):
        ...
        for url in product_requests:
            yield Request(url=urljoin(response.url, url), cookies=self.cookies, callback=self.parse_item,
    ...
    def parse_item(self, response):
        ...
        item['meta'] = {'requests': self.product_skus_requests(response, item)}

        item = self.next_item_or_request(item)
        return item

    def product_retailer_sku(self, response):
        sku_json_data = self.load_sku_data(response)
        return sku_json_data.get('displayCode')

    def product_name(self, response):
        return response.css('.product-name::text').get()

    def product_gender(self, response):
        raw_gender_text = response.css('.product-detail-section-text::text').get()
        for raw_gender_text in raw_gender_text:
            if raw_gender_text in GENDERS:
                return raw_gender_text
        return 'unisex'
    ...
    def product_skus_requests(self, response, item):
        requests = []
        selector = '.color-value:not(.selected)::attr(data-attr-value)'
        product_sku_variant = response.css(selector).getall()

        for variant_color in product_sku_variant:
            url = re.sub(r'[A-Z]{3,}', variant_color, response.url, )
            requests.append(
                Request(url=url, callback=self.update_product_skus, meta={'item': item})
            )
        return requests
    ...
    def product_images(self, response):
        ...
        raw_images_json = json.loads(raw_images)
        large_sized_images = raw_images_json.get('large')
    ...
    def product_sku(self, response):
        ...
        for product_size in product_sizes:
            skus.append(
                {"colour": sku_json_data.get('displayValue'),
                 "previous_prices": [self.product_previous_price(response)],
                 "size": product_size,
    ...
    def load_sku_data(self, response):
        ...
        sku_json_data = json.loads(raw_data)
        return sku_json_data
