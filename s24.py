...
class _24sParseSpider(Spider):
    ...
    def color_requests(self, response):
        ...
        return [Request(add_or_replace_parameter(response.url, 'color', color), callback=self.parse_color)
...
class _24sCrawlSpider(CrawlSpider):
    ...
    request_body_json = json.loads('{"requests":[{"indexName":"prod_products","params":"query=&tagFilters={}&page=0&'
                                   'getRankingInfo=true&ruleContexts={}"}]}')

    listings_css = ['.nav-link', '[class="device-drop-btn"]']
    deny = ['brands', 'le-bon-marche', 'your-24s']

    rules = (
        Rule(LinkExtractor(restrict_css=listings_css, deny=deny), callback='parse_categories'),
    )

    def parse_categories(self, response):
        payload = self.request_payload(response)
        return Request(self.pagination_url, method='POST', body=payload,
    ...
    def request_payload(self, response):
        ...
        raw_params = self.request_body_json['requests'][0]['params']
        params = raw_params.format(tag_filter, rule_contexts)
        self.request_body_json['requests'][0]['params'] = params

        return json.dumps(self.request_body_json)

    def get_tag_filters(self, response):
        raw_tag_filter = ["US"] + response.css('::attr(data-filter)').split(',')
    ...
    def request_body(self, response, page_number):
        ...
        raw_params = body['requests'][0]['params']
        params = add_or_replace_parameter(raw_params, 'page', page_number)
    ...
    def parse_products(self, response):
        ...
        for product in raw_products['hits']:
            product_url = f'{product["title_en"]}-{product["brand"]}_{product["item_group_id"]}'
            args = {
                'defaultSku': product['sku'],
                'color': product['color_brand']
            }
            product_url = add_or_replace_parameters(product_url, args)

            yield response.follow(self.clean_url(product_url), callback=self.parse_spider.parse)
