    ...
    def __init__(self, weather_reading):
        if weather_reading:
            if 'PKT' in weather_reading:
                self.full_date = datetime.strptime(weather_reading['PKT'], '%Y-%m-%d').date() 
...
class WeatherAnalyze:
    ...
    def display_yearly_report(self, file_data):
        ...
        date = self.get_required_date(file_data,mean_humidity,reverse_flag=True)
    ...
    def get_mean_average_humidity(self, file_data):
        humidity_mean_list = [file_rows.mean_humidity for file_rows in file_data if file_rows]
        return len(humidity_mean_list)
    
    def get_highest_average_temp(self, file_data):
        mean_list_max = [file_rows.max_temp for file_rows in file_data if file_rows]
        return round(sum(mean_list_max)/len(mean_list_max))

    def get_lowest_average_temp(self, file_data):
        mean_list_min = [file_rows.min_temp for file_rows in file_data if file_rows]    
    ...
    def reading_file(self, file_names):
        for file_name in file_names:
            with open(file_name, 'r') as csvfile:
                ...
                weather_readings = [WeatherData(file_row) for file_row in weather_file_readings]
