                ...
                try:
                    ...
                    for evaluate in evaluations:
                        ...
                        if key_header not in data_dict[year]:
                            data_dict[year][key_header] = {value_key: temp, date_key: date}
                            continue

                        if operation(temp, data_dict[year][key_header][value_key]) or not data_dict[year][key_header][value_key]:
                            data_dict[year][key_header] = {value_key: temp, date_key: date}
...
def print_report(data_dict, report_no):
    evaluations = get_evaluation_header()
    if report_no == 1:
        print_annual_min_max(evaluations, data_dict)
    elif report_no == 2:
...
def print_annual_min_max(evaluations, data_dict):
    ...
    for key_year, eval_dict in data_dict.items():
        print ('%s\t\t%s\t\t\t%s\t\t\t%s\t\t%s' % (str(key_year),eval_dict[evaluations[0][0]]["value"],eval_dict[evaluations[1][0]]["value"],eval_dict[evaluations[2][0]]["value"],eval_dict[evaluations[3][0]]["value"]))
...
def annual_evaluation_day(data_dict, evaluate):
    ...
    print ('------------------------------------------------------------')
    for key_year, eval_dict in data_dict.items():
        print ('%s\t\t%s\t\t\t%s' % (str(key_year),eval_dict[evaluate]["date"],eval_dict[evaluate]["value"]))


def validate(report_no=0, data_dir=''):
    try:
        val = int(report_no)
...
def main():
    # report_no = input("Enter Report # : ")
    # data_dir = input("Enter Data Directory : ")

    report_no = 3
    data_dir = '/Users/danish/Documents/Python/Assignment_1/weatherdata/'
    if validate(report_no, data_dir):
        data_dict = prepare_data_for_report(data_dir)
        print_report(data_dict, report_no)
