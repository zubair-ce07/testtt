...
import populate_weather_readings
...
def main():
    ...
    for i in islice(range(len(sys.argv)),2,None):
        if sys.argv[i] in ["-a","-c"]:
            ...
            weather_file_parser = populate_weather_readings.WeatherReadingsPopulator()
            weather_file_parser.list_files(given_path, sys.argv[i-1], given_year, given_month)
            ...
            weather_file_parser = populate_weather_readings.WeatherReadingsPopulator()
            weather_file_parser.list_files(given_path, sys.argv[i-1], given_year)
            weather_file_parser.populate_weather_readings(given_path)
            print(weather_file_parser.weather_readings)
            print ("*******************************\n")
    exit()
