...
class Calculations:
	...
	def calculations_for_command_e(self):
		parsed_data = self.file_parser.parse_file()

		if(parsed_data is not None):
			#Highest temperature and day
			self.calculation_results["Highest_temp"] = max([int(k["Max TemperatureC"]) for k in parsed_data if k["Max TemperatureC"]])
			self.calculation_results["Highest_temp_day(s)"] = [k["PKT"] for k in parsed_data if k["Max TemperatureC"] == str(self.calculation_results["Highest_temp"])]
			#Lowest temperature and day	
			self.calculation_results["Lowest_temp"] = min([int(k["Min TemperatureC"]) for k in parsed_data if k["Min TemperatureC"]]) 
			self.calculation_results["Lowest_temp_day(s)"] = [k["PKT"] for k in parsed_data if k["Min TemperatureC"] == str(self.calculation_results["Lowest_temp"])]
			#Most humidity and day
			self.calculation_results["Max_humidity"] = max([int(k["Max Humidity"]) for k in parsed_data if k["Max Humidity"]]) 
			self.calculation_results["Max_humidity_day(s)"] = [k["PKT"] for k in parsed_data if k["Max Humidity"] == str(self.calculation_results["Max_humidity"])]
	...
	def calculations_for_command_a(self):	
		...
		if(parsed_data is not None):
			...
			self.calculation_results["Avg_highest_temp"] = int(sum([int(k["Max TemperatureC"]) for k in parsed_data if k["Max TemperatureC"]])
															/len([int(k["Max TemperatureC"]) for k in parsed_data if k["Max TemperatureC"]]))
			#Average lowest temperature
			self.calculation_results["Avg_lowest_temp"] = int(sum([int(k["Min TemperatureC"]) for k in parsed_data if k["Min TemperatureC"]])
															/len([int(k["Min TemperatureC"]) for k in parsed_data if k["Min TemperatureC"]]))
			#Average Mean Humidity
			self.calculation_results["Avg_mean_humidity"] = int(sum([int(k[" Mean Humidity"]) for k in parsed_data if k[" Mean Humidity"]])
															/len([int(k[" Mean Humidity"]) for k in parsed_data if k[" Mean Humidity"]]))
			return self.calculation_results
	...
	def calculations_for_command_c(self):
		...
		if(parsed_data is not None):
			...
			self.calculation_results["Highest_temp_record"] = [k["Max TemperatureC"] for k in parsed_data]
			#List of Lowest Temperatures on each day
			self.calculation_results["Lowest_temp_record"] = [k["Min TemperatureC"] for k in parsed_data]
			return self.calculation_results
		else:
			return None	
...
def main():
	#x = ReportGenertaor()
	#x.report_generation_for_command_e(r"C:\Users\Alishba Naeem\Documents\Lab 1\the-lab\weatherfiles\weatherfiles", "2002")
