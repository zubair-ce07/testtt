...
class Bonus:
    """
    This class show report of max and min in one chart in red and blue
    """
    ...
    def __init__(self,year,month):
        self.year = year
        self.month = month
        self.formated_month = datetime(int(self.year), 
                                        int(self.month),
                                         1)
        self.selected_month = self.formated_month.strftime("%b")
        self.filname = ('Murree_weather_' 
                        + str(self.year) 
                        + '_' 
                        + self.selected_month 
                        + '.txt')
...
class Month_Year:
    """
    This class calculate the averaging of max temp, min temp and mean humidity from a file
    """
...
class Monthly_Report:
    """
    This class show a monthly report into two different charts
    """
...
class Yearly_report:
    """
    This class show a yearly report
    """
    ...
    def yearly_graph_for_maximum(self):
        basepath1 = r"/home/ahmed/the-lab/Ahmad_weatherman"
        for entry1 in os.listdir(basepath1):
            if os.path.isfile(os.path.join(basepath1,entry1)):
                if(entry1.startswith(self.filename 
                                    + self.year)):
...
class Year:
    """
    This class calculate max temp, min temp and humidity from all given year files
    """
    ...
    def Maximum_Temperature(self):
        ...
        store_values2 = []
        store_values2 = list(max(common_entries(self.store_values,
                                                self.store_values1),
    ...
    def Minimum_Temperature(self):
        ...
        store_values3 = []
        store_values3 = list(min(common_entries(self.store_values2,
                                                self.store_values3),
                                                key=lambda item:item[1]))
    ...
    def Humidity(self):
        ...
        store_values4 = []
        store_values4 = list(min(common_entries(self.store_values4,
                                                self.store_values5),
                                                key=lambda item:item[1]))
...
def main():
    ...
    if option == '1':
        for i in string_argv:
            if(not((i.endswith("py")) or (i.endswith("-c")) or (i.endswith("a")) or (i.endswith("-e")))):
                values = i.split("/")
                if(len(values) > 1):
                    year = values[0]
                    month = values[1]
