...
class FarfetchSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        :return:
        """
        yield scrapy.Request(url=response.url, headers={
    ...
    def parse_product(self, response):
        ...
        if raw_data:
            ...
            product['composition'] = self.get_composition(raw_data)
            product['care'] = self.get_care(raw_data)
            product['bread-crumb'] = self.get_bread_crumb(raw_data)
    ...
    def get_bread_crumb(self, raw_data):
        bread_crumbs = []
    ...
    def get_composition(self, raw_data):
        composition = []
    ...
    def skus_formation_price(self, size_id, raw_data):
        try:
            ...
            return raw_data.get('productViewModel').get('priceInfo').get('default').get('finalPrice')
