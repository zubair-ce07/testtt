...
class WeatherMan: 
    ...
    if __name__ == '__main__':
        ...
        if result.a:
            ...
            results = Calculations.calulate_average_record(file_readings)
            Report.show_monthly_report(results)
            print()
        if result.e:
            print(f"Report for year {result.e[0]}")
            files_to_parse = []
            file_readings = Parser.parse_files(directory,result.e[0])
            results = Calculations.calulate_year_wise_record(file_readings)
            Report.show_yearly_report(results)
            print()
        if result.c:
            files_to_parse = []
