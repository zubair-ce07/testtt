...
def main():
    ...
    if not os.path.exists(sys.argv[1]):
        ...
        while index < len(sys.argv):
            ...
            flat_iter = 0  # Variable to iterate over flag_indexes
            while flat_iter < len(flag_indexes):
                tokenize_date = sys.argv[flag_indexes[flat_iter] + 1].split('/')
            ...
            flat_iter = 0
            while flat_iter < len(flag_indexes):
                if int(sys.argv[flag_indexes[flat_iter] + 1]) not in range(2004, 2017):
                    logging.error('Year must between 2004 - 2016')
                    exit(0)
                reading_list = list()
                month = 1
                while month <= 12:
                    path = \
                        sys.argv[1] + '/weatherfiles/Murree_weather_' + sys.argv[flag_indexes[flat_iter] + 1] + \
