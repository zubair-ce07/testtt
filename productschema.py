...
def parse_price(selector):
    price = selector.css('.special-price>span[class=price]::text').extract_first() \
        or selector.css('.regular-price>span::text').extract_first()
    return re.search(r"\d+\.\d+", price).group()
...
def parse_description(selector):
    description = selector.xpath('//div[@class="short-description-value"]//text()').extract()
    return ''.join(''.join(description).strip().splitlines()).split(". ")
...
def single_product(selector):
    return {
        "colour":          "one colour",
        "currency":        re.search(r"\w*", selector.css('.price::text').extract_first()).group(),
        "out_of_stock":    False,
        "size":            "one size",
    ...
    if re.search(r'"@type": *"Product"', json_string):
        ...
        for raw_sku in json.loads(json_string):
            ...
            yield {
                ...
                "out_of_stock":    "InStock" in raw_sku.get("offers").get("availability"),
...
def parse_product(response):
    ...
    return {
        ...
        "care":         [],
        "brand":        selector.css('.branding>strong::text').extract_first(),
        "url":          response.url,
