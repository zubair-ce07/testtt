...
class GymboreeSpider(CrawlSpider):
    ...
    rules = (
        Rule(LinkExtractor(restrict_xpaths="//ul[contains(@class, 'nav') and contains(@class, 'nav-pills')]/li/a")),
        Rule(LinkExtractor(restrict_xpaths="//li/a[@foldertype='categoryLink']")),
        Rule(LinkExtractor(restrict_xpaths='//a[@class="next"]')),
        Rule(LinkExtractor(restrict_xpaths='//div[@class="product-name"]/a'),
    ...
    def get_item_currency(self, response):
        return response.xpath(
            "//div[@id='pdp-price']/meta/@content").extract_first()
    ...
    def get_item_gender(self, response):
        trail_checkpoints = response.xpath(
    ...
    def get_item_price(self, response):
        return response.xpath(
            "//span[@id='pdp-regular-price']/text()").extract_first()

    def get_item_retailer(self):
        return 'gymboree'

    def get_item_sku(self, response):
        return response.xpath("//span[@itemprop='mpn']/text()").extract_first()
    ...
    def get_item_url_trail(self, response):
        return response.xpath(
            ".//*[@id='pdp-breadcrumbs']/li/a/@href").extract()
