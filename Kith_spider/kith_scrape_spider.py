...
class KithSpider(scrapy.Spider):
        ...
        if last_page_number:
            ...
            for index in range(2, last_page_number):
                if response.url.endswith('/'):
                    urls.append("{}?page={}".format(response.url[:-1], str(index)))
                else:
                    urls.append("{}?page={}".format(response.url, str(index)))
        for url in urls:
    ...
    def parse_sizes(self, response):
        return response.xpath('//div[contains(@class,"product-single-form-item -dropdown -full")]/select/option/text()').extract()
    ...
    def parse_description(self, response):
        ...
        if description_list:
            ...
            description = response.xpath('//div[contains(@class, "product-single-details-rte rte mb0")]/ul/li/text()').extract()
