...
class GarageSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for my_url in urls:
            ...
            yield scrapy.Request(url=my_url,
    ...
    def parse_cloth(self, response):
        yield {
            "name": response.css('h1.prodName::text').extract_first(),
            "price": response.css('h2.prodPricePDP::text').extract_first(),
            "promo_message": response.css('p.promoMessage::text').extract_first(),
            "color": response.css('div.swatchColor::text').extract_first(),
            "sizes": response.css('div#productSizes span::text').extract(),
            "description": response.css('div#descTabDescriptionContent>p::text').extract_first(),
        }
