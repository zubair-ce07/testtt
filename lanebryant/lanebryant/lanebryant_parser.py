...
class ProductParser(Spider):
    ...
    def extract_image_urls(self, response, base_url):
        raw_image_urls = json.loads(response.css("p::text").re("{.*}")[0])
        return [urljoin(base_url, url["i"]["n"]) for url in raw_image_urls["set"]["item"]]
    ...
    def extract_category(self, response):
        ...
        raw_categories = json.loads(response.css(category_css).re("{.*}")[0])
