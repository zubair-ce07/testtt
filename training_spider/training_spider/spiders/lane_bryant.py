...
class AdidasSpider(CrawlSpider):
    ...
    def parse_pagination(self, response):
        try:
            ...
            product_grid = HtmlResponse(
                ...
                encoding='utf-8')
            for url in product_grid.css('.mar-prd-item-image-container::attr(href)').extract():
                yield Request('http://www.lanebryant.com{}'.format(url),
            ...
            next_page_path = json.loads(response.css('#endpoints::text').extract_first())\
                .get('data').get('endpoints').get('plpData')
            url_params = response.css('#nextPageUrlOld::attr(value)').\
                re_first('\?N=\d+&No=\d+')
            next_page_url = '{}{}'.format(next_page_path, url_params)
            yield Request(response.urljoin(next_page_url), callback=self.parse_pagination)
    ...
    def parse_products(self, response):
        ...
        product = json.loads(response.css('#pdpInitialData::text')
                             .extract_first()).get('pdpDetail').get('product')[0]
        ...
        colors = [self.get_colors(color, sizes, server_url, account_id)
    ...
    def request_image_json_urls(self, colors, item, server_url, variations=None):
        if colors:
            color = colors.pop(0)
            return Request(color.get('images_josn_urls'), callback=self.parse_images_json_urls,
                           meta={'color': color, 'remaining_colors': colors, 'item': item,
                                 'server_url': server_url, 'variations': variations})
        else:
    ...
    def parse_images_json_urls(self, response):
        ...
        if isinstance(images_json, list):
            image_urls = ['http:{}{}'.format(server_url, img_json.get('i').get('n'))
                          for img_json in images_json]
        else:
            image_urls = 'http:{}{}'.format(server_url, images_json.get('i').get('n'))
    ...
    def get_colors(self, color, sizes, server_url, account_id):
        ...
        return {'color_id': color_id, 'color_name': color.get('name'),
                ...
                'images_josn_urls': 'http:{}{}/{}?req=set,json&id={}'.format(
                    server_url, account_id, color.get('imageset'), color_id),
                'main_image': 'http:{}'.format(color.get('sku_image'))}

    def get_sizes(self, sizes, skus):
        return [
            {key: {'name': size.get('value'),  'sale_price': sku.get(key).get('sale_price'),
                   'price': sku.get(key).get('price'), 'is_available': True}}
    ...
    def get_skus(self, skus):
        return [{'{}_{}'.format(sku.get('color'), sku.get('size')):
                 ...
                 'sale_price': sku.get('prices').get('sale_price') or '-'}} for sku in skus]
