...
class WeatherDataAnalysis:
    ...
    def update_yearly_highest_temp(self, report):
        max_temp_list = report['month_data'].attribute_values('Max TemperatureC')
        ...
        if year_max_temp <= month_max_temp:
            ...
            date_value = datetime.datetime.strptime(report['date_values'][row], '%Y-%m-%d')
            report['highest_temp'] = month_max_temp
            report['highest_temp_date'] = date_value

    def update_yearly_lowest_temp(self, report):
        min_temp_list = report['month_data'].attribute_values('Min TemperatureC')
    ...
    def update_yearly_most_humidity(self, report):
        highest_humidity_list = report['month_data'].attribute_values('Max Humidity')
        year_highest_humidity = report['highest_humidity']
        month_highest_humid_day = max(list(filter(None, highest_humidity_list)))
    ...
    def calculate_monthly_record(self, weather_readings, date):
        ...
        max_temp_values = month.attribute_values('Max TemperatureC')
        min_temp_values = month.attribute_values('Min TemperatureC')
        mean_humidity_values = month.attribute_values('Mean Humidity')
        ...
        total_mean_humidity_records = len(list(filter(None, mean_humidity_values)))
        report['operation'] = 'a'
        report['average_max_temp'] = (max_temp_sum / total_max_temp_records)
        report['average_min_temp'] = (min_temp_sum / total_min_temp_records)
        report['average_mean_humidity'] = (mean_humidity_sum / total_mean_humidity_records)
        return report
