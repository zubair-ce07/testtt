...
class ClothesSpider(CrawlSpider):
    ...
    rules = (
        Rule(LinkExtractor(restrict_css='.thmNm', allow='/p'), callback='parse_item'),
        ...
        if not cat_requests:
            cat_requests.append(
                FormRequest(f"https://www.softsurroundings.com/p/{self.extract_retailer_sku(response)}",
                            method='POST', callback=self.color_requests))
        return cat_requests
    ...
    def color_requests(self, response):
        ...
        for c_id in color_ids:
            color_requests.append(FormRequest(f'{response.url}{c_id}', method='POST', callback=self.extract_skus))
