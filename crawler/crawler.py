...
class Scheduler:
    ...
    def crawl_parallel(self):
        ...
        return (self.bytes_downloaded.value,
    ...
    def fetch_next_urls(self, fetch, raw_response_status_code, raw_response_text):
        if raw_response_status_code == 200:
            self.bytes_downloaded.value += len(raw_response_text)
            self.pages_crawled.value += 1
...
class CrawlerReport:
    def __init__(self, bytes_downloaded, pages_crawled, errors):
        self.bytes_downloaded = bytes_downloaded
        self.pages_crawled = pages_crawled
        self.errors = errors

    def print_crawler_report(self, start_time):
        print(f"Total bytes downloaded: {self.bytes_downloaded}")
        print(f"Total pages crawled: {self.pages_crawled}")
        print(f"Average page size: {self.bytes_downloaded / self.pages_crawled}")
