...
def display_oneline_year_graph(year_record_list, year, path):
    ...
    for month in FILE_MONTHS:
        ...
        if year_record_list is not None and len(year_record_list) > 0:
            max_temp_entry = max(([y for y in year_record_list if y.temp.max_temp is not None]),
                                 key=lambda x: x.temp.max_temp)
            min_temp_entry = min(([y for y in year_record_list if y.temp.min_temp is not None]),
...
def calculate_year_report_from_list(year_record_list):
    ...
    min_temp_entry = min(([y for y in year_record_list if y.temp.min_temp is not None]),
                         key=lambda x: x.temp.min_temp)
    humidity_entry = max(([y for y in year_record_list if y.humidity.max_humidity is not None]),
    ...
    year_report = YearReport(max_temp_entry.temp.max_temp, max_temp_entry.date,
                             ...
                             humidity_entry.humidity.max_humidity, humidity_entry.date)
    return year_report
...
def display_year_report(path, year, graph):
    ...
    if graph is False:
        ...
        if year_record_list is not None and len(year_record_list) > 0:
            ...
            print("\n<< Data is not avialbe for given year. [Required: year]\n")
    else:
