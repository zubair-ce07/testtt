...
from CalculateResults import ResultCalculator
...
def process_yearly_temperatures(path, year):
    ...
    results = Report()
    readings = f_parser.populate_yearly_temps(path, year)
...
def process_monthly_avgs(path, month):
    ...
    result_calculator = ResultCalculator()
    results = Report()
    temp_readings = f_parser.populate_monthly_temps(path, month)
