...
class WeatherResultsCalculator:
    ...
    def calculate_monthly_avg_weather(self, weather_readings):
        avg_high_temperature = sum([reading.high_temperature for reading in weather_readings]) / \
                               len(weather_readings)
        avg_low_temperature = sum([reading.low_temperature for reading in weather_readings]) / \
                              len(weather_readings)
        avg_mean_humidity = sum([reading.mean_humidity for reading in weather_readings]) / \
                            len(weather_readings)
