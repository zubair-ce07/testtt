...
from constants import (CRED, CBLUE, CEND, DEFAULT, FILE_MONTHS)
...
def calculate_month_avg_file(filename, rec_list, month_str):
    ...
    if rec_list is not None and len(rec_list) > 0:
        ...
        humidity_avg = sum(humidity_avg_data) / len(humidity_avg_data)
...
def display_month_report(path, year_month, graph):
    ...
    try:
        ...
        if month > -1 and month < 12:
            file_name = get_path_file(path, year, FILE_MONTHS[month])
            month_record_list = []
            if graph is False:
