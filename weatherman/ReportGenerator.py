    ...
    def draw_bar_charts(self):
        ...
        for row in self.weather_data_obj.weather_readings:
            if row.max_temperature and row.min_temperature:
                ...
                highest = f"\033[0;34;50m{'+' * row.max_temperature if row.max_temperature > 0 else '-' * row.max_temperature}"
                lowest = f"\033[0;31;50m{'+' * row.min_temperature if row.min_temperature > 0 else '-' * row.min_temperature}"
