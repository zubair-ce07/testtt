...
class WeatherReport:
    ...
    def print_yearly_report(self, highest_temp_day, lowest_temp_day, highest_humidity_day):
        ...
        print('Highest: ' + highest_temp_day.max_temperature + 'c on '
              + self.__get_month_name(highest_temp_day.PKT.split('-')[1])
              + ' ' + highest_temp_day.PKT.split('-')[2])

        print('Lowest: ' + lowest_temp_day.min_temperature + 'c on '
              + self.__get_month_name(lowest_temp_day.PKT.split('-')[1])
              + ' ' + lowest_temp_day.PKT.split('-')[2])

        print('Humidity: ' + highest_humidity_day.max_humidity + '% on '
              + self.__get_month_name(highest_humidity_day.PKT.split('-')[1])
              + ' ' + highest_humidity_day.PKT.split('-')[2])

    def get_month_data(self, year_and_month, files_path):
        month_data = []
        year = year_and_month.split('/')[0]
        month = year_and_month.split('/')[1]
        months_key = {v: k for k, v in self.MONTHS_KEY_MAP.items()}
        ...
        month_name = self.MONTHS_NAMES[self.MONTHS_KEY_MAP[int(month)]]
