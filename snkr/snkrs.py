...
class SnkrsSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for url in urls:
            yield response.follow(url, callback=self.listing_page)
    ...
    def listing_page(self, response):
        urls = response.css('ul.product_list.grid.row' +
            ...
            ' > .product_img_link::attr(href)').getall()
    ...
    def get_gender(self, response):
        gender = response.css('div.nosto_category::text').re_first(r'(Men|Women|Skate|Lifestyle)')
        ...
        elif (gender != 'Men') or (gender != 'Women'):
            gender = 'unisex-adults'
            return gender

    def get_url(self, response):
        return response.css('div.nosto_product > span.url::text').get()

    def get_name(self, response):
        return response.css('div.nosto_product > span.name::text').get()

    def get_image_urls(self, response):
        return  response.css('div.nosto_product > .image_url::text').getall() + \
                response.css('div.nosto_product > .alternate_image_url::text').getall()
