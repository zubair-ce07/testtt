...
def merge_records(primary_records, secondary_records):
    records = []
    for key in secondary_records.keys():
        record = {}
        record['Title'] = unquote(''.join(re.findall(r'q=(.*)\+', primary_records[key][0])))
        record['Location'] = unquote(''.join(re.findall(r'\+(.*?)&', primary_records[key][0])))
        record['Google URL'] = primary_records[key][0]
        ...
        if key in primary_records:
            for sc_record in secondary_records[key]:
                ...
                new_record['Rank'] = primary_records[key][5]
