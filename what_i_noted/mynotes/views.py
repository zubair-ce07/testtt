...
from . import constants as note_app_constants
...
class PublicHomePageListView(generic.ListView):
    ...
    template_name = 'mynotes/public/public_home.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update({
            'notes': Note.objects.filter(is_public=True).order_by(
                note_app_constants.ORDER_BY_UPDATED_AT)[:note_app_constants.PAGE_LIMIT_FOUR]
        })
        return context
...
def search_by_keywords(request):
    """This method is returning note books and notes which matching the query
     ...
     Note Book title
     """
    template_name = 'mynotes/search_notes.html'
...
def search_books(request):
    ...
    template_name = 'mynotes/search_books.html'
    query = request.GET.get('q')
    context = {}

    if query:
        response = requests.get(note_app_constants.GOOGLE_BOOKS_API_URL+query)
        if response.status_code == 200:
            response_data = json.loads(response.content)
            context = {'message': note_app_constants.SUCCESS_MESSAGE, 'books': response_data['items']}
        elif response.status_code == 404:
            context = {'message': note_app_constants.NO_DATA_FOUND_MESSAGE}
    return render(request, template_name, context)


class AllPublicNoteBookListView(generic.ListView):
    """Showing the list of all public note books"""
    template_name = 'mynotes/public/public_note_books.html'
    context_object_name = 'note_books'
    paginate_by = note_app_constants.PAGE_LIMIT_EIGHT

    def get_queryset(self):
        return NoteBook.objects.filter(is_public=True).order_by(note_app_constants.ORDER_BY_UPDATED_AT)


class AllPublicNoteListView(generic.ListView):
    """Showing the list of all public notes"""
    template_name = 'mynotes/public/public_notes.html'
...
class HomePageListView(LoginRequiredMixin, generic.ListView):
    ...
    def get_queryset(self):
        return NoteBook.objects.filter(user=self.request.user).order_by(*note_app_constants.ORDER_BY_UPDATED_AT_AND_IS_FAVORITE)


class CreateNoteBook(LoginRequiredMixin, generic.CreateView):
    """Display the form for creating new note book"""
    model = NoteBook
    template_name = 'mynotes/notebook_create.html'
    ...
    def form_valid(self, form):
        ...
        messages.success(self.request, note_app_constants.NOTE_BOOK_CREATED_MESSAGE)
        # Custom signal
        note_book_created_signal.send(NoteBook, message=note_app_constants.NOTE_BOOK_CREATED_MESSAGE)
...
class CreateNote(LoginRequiredMixin, generic.CreateView):
    ...
    model = Note
    template_name = 'mynotes/note_create.html'
    ...
    def form_valid(self, form):
        ...
        messages.success(self.request, f'{note_app_constants.NOTE_CREATED_MESSAGE}{form.instance.note_book.title}!')
...
class NoteListView(LoginRequiredMixin, generic.ListView):
    ...
    template_name = 'mynotes/notes.html'
    model = Note
    context_object_name = 'notes'
    paginate_by = note_app_constants.PAGE_LIMIT_EIGHT
    ...
    def get_queryset(self):
        return Note.objects.filter(note_book_id=self.kwargs.get('pk')).order_by(
            *note_app_constants.ORDER_BY_UPDATED_AT_AND_IS_FAVORITE)
...
class UpdateNoteBook(LoginRequiredMixin, generic.UpdateView):
    ...
    model = NoteBook
    template_name = 'mynotes/notebook_update.html'
    fields = ['title', 'author_name', 'publisher_name', 'published_date',
              'isbn', 'thumbnail', 'is_public', 'is_favorite']

    def form_valid(self, form):
        form.instance.user = self.request.user
        messages.success(self.request, note_app_constants.NOTE_BOOK_UPDATE_MESSAGE)
        return super().form_valid(form)
...
class UpdateNote(LoginRequiredMixin, generic.UpdateView):
    ...
    model = Note
    template_name = 'mynotes/note_update.html'
    fields = ['title', 'description', 'tags', 'thumbnail', 'is_public', 'is_favorite']

    def form_valid(self, form):
        messages.success(self.request, note_app_constants.NOTE_UPDATE_MESSAGE)
        return super().form_valid(form)
...
class AboutPageListView(generic.ListView):
    ...
    template_name = 'mynotes/about.html'
...
class NotePublicView(generic.DetailView):
    ...
    model = Note
    template_name = 'mynotes/note_view.html'


class NotePublicListView(generic.ListView):
    """ Displays list of all public notes  """
    template_name = 'mynotes/public/notes_public_view.html'
    model = Note
    context_object_name = 'notes'
    paginate_by = note_app_constants.PAGE_LIMIT_EIGHT
    ...
    def get_queryset(self):
        return Note.objects.filter(note_book_id=self.kwargs.get('pk'), is_public=True).order_by(
            note_app_constants.ORDER_BY_UPDATED_AT)
...
def add_google_lib_book(request):
    ...
    if request.is_ajax():
        ...
        note_book.save()
        messages.success(request, note_app_constants.NOTE_BOOK_CREATED_MESSAGE)
        return HttpResponse(note_app_constants.NOTE_BOOK_CREATED_MESSAGE)
    else:
        messages.success(request, note_app_constants.SOMETHING_WENT_WRONG_MESSAGE)
        return HttpResponse(note_app_constants.SOMETHING_WENT_WRONG_MESSAGE)
