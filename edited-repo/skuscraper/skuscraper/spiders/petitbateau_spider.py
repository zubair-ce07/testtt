...
class PetitBateauParseSpider(BaseParseSpider, Mixin):
    ...
    listing_url = 'https://www.petit-bateau.co.uk/WebServices/CatalogService.asmx/GetVariant'
    listing_payload_t = '{{"productId": "{}","variantId":"{}", "selectedSize":"{}"}}'
    ...
    def parse(self, response):
        ...
        response.meta['image_source'] = self.image_data_source(response)
        garment['skus'] = {}
        garment['image_urls'] = []
    ...
    def parse_colours(self, response):
        ...
        requests_queue += self.size_requests(sizes, product_id, variant_id)
    ...
    def request_payloads(self, url):
        ...
        return self.listing_payload_t.format(*ids, "")
    ...
    def colour_requests(self, response):
        ...
        for url in self.colour_urls(response):
            ...
            request = FormRequest(url=self.listing_url, callback=self.parse_colours,
                                  method="POST", headers=self.headers, body=request_body)
            request.meta['image_source'] = response.meta['image_source']
    ...
    def size_requests(self, sizes, product_id, variant_id):
        size_requests = []

        for size in sizes:
            request_body = self.listing_payload_t.format(product_id, variant_id, size)
            request = FormRequest(url=self.listing_url, callback=self.parse_skus,
                                  method="POST", headers=self.headers, body=request_body)
...
class PetitBateauCrawlSpider(BaseCrawlSpider, Mixin):
    ...
    def parse(self, response):
        ...
        if category and refining_id:
            yield self.request_lisitng(category[0], refining_id[0])
    ...
    def payload_category(self, response):
