# -*- coding: utf-8 -*-
"""This file contains scrapper of eloquii.com"""
import re
import scrapy
...
class EloquiiSpider(scrapy.Spider):
    """This is the class of scrapper
    that scrape the eloquii.com"""
    ...
    def parse_products(self, response):
        """This method extract all the links of product
        and send request to all links to get details"""

        product_links = response.css(
            "div.product-images a::attr(href)"
            ).extract()
        ...
        # pagination
        #
        next_page_sel = response.css(
            "div.row.justify-content-center.mt-5 div:nth-child(3)"
            )
        if next_page_sel:
    ...
    def parse_product_details(self, response):
        """This method extract all the details of
        product and fill the item"""

        product = EloquiiProduct()

        product["product_id"] = re.search(
            r'(\d+)\.html', response.url
        ).group(1)
        ...
        product["category"] = response.xpath(
            "//*[@id='bt_pdp_main']/div[1]/div/span[3]/span/text()"
            ).extract_first()

        product["description"] = response.xpath(
            "//*[@id='bt_pdp_main']/div[2]/div/div/div[3]/div/div[3]/div\
            /div/div/div[3]/span/text()"
            ).extract_first()
        ...
        merch_info = response.css(
            "h3.text-24.font-demi.mb-3.text-center::text"
            ).extract_first()
    ...
    def get_img_urls(self, response):
        """This method extract the all image urls
        and convert small images to large"""

        raw_img_url = response.css(
            "div.swiper-wrapper.productthumbnails div img::attr(src)"
            ).extract()
    ...
    def get_product_skus(self, response):
        """This method extract the skus of product
        if product is available"""

        skus = {}
        colors_list = response.css(
            "ul.swatchesdisplay.list-inline li a::attr(title)"
            ).extract()
        sizes_list = response.css(
            "div#product_detail_size_drop_down_expanded a::attr(title)"
            ).extract()
        sizes_list = sizes_list[1:]
        price = response.css("span.subprice-retail::text").extract_first()
        if not price:
            price = response.css(
                "div.priceGroup span.text-pink::text"
                ).extract_first()
