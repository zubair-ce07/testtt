...
class KayseriaComSpider(scrapy.Spider):
    ...
    def get_item_sizes(self, response):
        sizes = response.xpath(
            "//ul[@id='configurable_swatch_size']//span[@class='swatch-label swatch-label_hover']/text()").extract()
        return [size.strip() for size in sizes]

    def get_item_skus(self, response):
        color_name = response.xpath(
            "//span[@id='select_label_']/text()").extract_first()
        if not(color_name):
            attrib_keys = response.xpath(
                "//table[@id='product-attribute-specs-table']/tbody/tr/th/text()").extract()[1:]
            attrib_values = response.xpath(
        ...
        price = response.xpath(
            "//span[contains(@id, 'product-price-')]//text()").extract_first().strip().strip("PKR ")
        if not(price):
            price = response.xpath(
                "//span[contains(@id, 'product-price-')]/span/text()").extract_first().strip().strip("PKR ")
