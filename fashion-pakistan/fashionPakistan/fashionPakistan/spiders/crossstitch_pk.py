...
class CrossstitchComSpider(scrapy.Spider):
    ...
    start_urls = [
        'https://www.crossstitch.pk/Common/SetCurrencyByIp?ip='+my_public_ip]
    ...
    def is_out_of_stock(self, response):
        ...
        value = response.xpath(
            "//input[@id='add-to-cart-button-{}']/@value".format(p_id))
    ...
    def get_item_sizes(self, response):
        sizes = response.xpath(
            "//select[contains(@id, 'product_attribute_')]/option/text()").extract()
        size_keys = response.xpath(
            "//select[contains(@id, 'product_attribute_')]/option/@value").extract()
    ...
    def get_item_skus(self, response):
        ...
        prev_price = response.xpath(
            "//div[@class='old-product-price']/span/text()").extract_first()
