...
urlpatterns = [
    ...
    url(r'^books/(?P<pk>[-\w]+)/renew/$',
        views.renew_book_librarian, name="renew-book-librarian"),
    url(r'^authors/create/$', views.AuthorCreate.as_view(), name='author-create'),
    url(r'^authors/(?P<pk>\d+)/update/$',
        views.AuthorUpdate.as_view(), name="author-update"),
    url(r'^authors/(?P<pk>\d+)/delete/$',
        views.AuthorDelete.as_view(), name='author-delete'),
    url(r'^books/create/$',
        views.BookCreate.as_view(), name='book-create'),
    url(r'^books/(?P<pk>\d+)/update/$',
        views.BookUpdate.as_view(), name='book-update'),
    url(r'^books/(?P<pk>\d+)/delete/$',
        views.BookDelete.as_view(), name='book-delete'),
    url(r'^books/(?P<fk>\d+)/create/$',
        views.BookInstanceCreate.as_view(), name='bookinst-create'),
    url(r'^books/(?P<fk>\d+)/(?P<pk>[-\w]+)/update/$',
        views.BookInstanceUpdateStatus.as_view(), name='bookinst-update-status'),
    url(r'^books/(?P<fk>\d+)/(?P<pk>[-\w]+)/delete/$',
        views.BookInstanceDelete.as_view(), name='bookinst-delete'),
]
