...
from ..parsers.genderparser import *
...
class BaseSpiderMixin:
    ...
    def initialize_mixins(self):
        ...
        # Only set lang if it's not None
        self.lang = get_spider_lang(self)
...
class BaseParseSpider(Spider, BaseSpiderMixin):
    ...
    care_materials = [
        ...
        'textile', 'polyurethan', 'synthetic', 'acrylic', 'rayon', 'polyester', 'composition',
        'rubber', 'vinyl', 'latex', 'lyocell', 'stainless steel', 'wood', 'lambskin',
        'metal', 'plastic', 'suede', 'shearling', 'denim', 'poly', 'flyknit', 'snakeskin', 'mesh',
        'ponte', 'poylester', 'palyacryl', 'acryl', 'sheepskin', 'satin', 'calfskin', 'lamb leather',
        'velvet', 'feathers', 'brass',

        # German
        'leder', 'textil', 'kunststoff', 'wolle', 'elasthan', 'futter', 'elastische', 'synthetik', 'schutz',
        'kaschmir', 'lammfell',
        ...
        # french
        'coton', 'cuir', 'synthetique', 'laine', 'daim',

        # Spanish
        'textil', 'goma', "composición",

        # Czech
        'polyamid', 'hedvábí', 'bavlna', 'vlna', 'kůže', 'kašmír', 'prádlo', 'viskóza',
        'elastan', 'textilní', 'polyuretan', 'syntetický', 'akryl',
        'umělé hedvábí', 'guma', 'nerezová ocel', 'dřevo', 'cachemira',
        ...
        # Italian
        'seta', 'lana', 'capretto', 'scamosciato', 'cashmere', 'cachemire','composizione',
        ...
        'ウール', 'シルク', 'シルク', 'ドライのみ', 'スワロフスキー', 'ロジウム', 'レザー', 'ステンレス', '合金', '樹脂', 'ウレタン',
        'ニッケル', 'プラスチック', '亜鉛', 'アルミニウム', 'レーヨン', 'アクリル', '金メッキ', '真鍮', 'ウール', 'レーヨン', '絹', 'レザー',

        # Lithuanian
        'poliuretaninis', 'medvilnė', 'kaučiukas', 'cheminiu',
    ...
    care_wash = [
        ...
        'bügeln', 'bleichen', 'dampf', 'temperatur', 'schonwaschgang', 'normalwaschgang',
        ...
        # Russian
        'стирка', 'химчистка', 'химическая', 'bбез пара', 'темп', 'гладить', 'сушилке', 'сушить', 'отбеливатели',
        ...
        # Japanese
        '洗う', '漂白', '洗濯機', 'ドライオンリー', '鉄', 'ポリカーボネート',

        # Chille
        'lavado', 'lavar', 'secado', 'lejía', 'cloro', 'cuidar',

        # Polish
        'chemicznie', 'prasować', 'bębnowej', 'suszarce', 'suszyć', 'bielić',
    ...
    token_gender_parser = None
    soup_gender_parser = None
    ...
    def gender_lookup(self, soup_or_lst, greedy=False, merge_genders=False, custom_gender_map=None,
                      use_default_gender_map=False, extract_first=False):
        """
        :param soup_or_lst: soup to be look on
        :param greedy: determines if a greedy search (with the "in" check) has to be made on the soup(value: True)
                        or token lookup has to be performed(value: False).
        :param merge_genders: merge genders like men and women in to unisex-adults (same for unisex-kids)
        :param custom_gender_map: A specialized gender_map if the search from soup has to be limited to
                                    only this gender_map
        :param use_default_gender_map: Works with SoupGenderParser (ignored in TokenGenderParser) - Use the default
                                        gender_map
        :param extract_first extracts the first gender found in the soup
        :return: looks up gender and returns either gender string or None

        greedy: The decision of using non-greedy approach can be decided on the gender terms for example if you
                do not have terms that contain spaces or any special characters in the gender_map, you should use
                non-greedy approach

        """
        flags = 0

        if use_default_gender_map:
            flags |= GenderParserFlags.USE_DEFAULT_GENDER_MAP

        if merge_genders:
            flags |= GenderParserFlags.MERGE_GENDERS

        if extract_first:
            flags |= GenderParserFlags.EXTRACT_FIRST_GENDER

        if greedy:
            if not self.soup_gender_parser:
                gender_map = getattr(self, 'spider_gender_map', {})
                self.soup_gender_parser = SoupGenderParser(gender_map)

            return self.soup_gender_parser.lookup(soup_or_lst, custom_gender_map, flags)

        if not self.token_gender_parser:
            use_en_map = getattr(self, 'use_en_gender_map', True)
            spider_lang = getattr(self, 'lang', 'en')
            gender_map = getattr(self, 'spider_gender_map', {})
            self.token_gender_parser = TokenGenderParser(gender_map, spider_lang, use_en_map)

        return self.token_gender_parser.lookup(soup_or_lst, custom_gender_map, flags)
    ...
    def product_pricing_common_new(self, response, money_strs=None, post_process=None, is_cents=False):
        common = {}
        pprices, price, currency = self.product_pricing_new(response, money_strs=money_strs,
                                                            post_process=post_process, is_cents=is_cents)
    ...
    def product_pricing_new(self, response, money_strs=None, post_process=None, locale=None, is_cents=False):
        ...
        return CurrencyParser.extract_prices(money_strs, locale=locale, is_cents=is_cents)
...
class BaseCrawlSpider(CrawlSpider, BaseSpiderMixin):
    ...
    def from_crawler(cls, crawler, *args, **kwargs):
        ...
        spider.load_akamai_headers(
            akamai_headers=os.getenv("akamai_headers"),
        )
    ...
    def load_akamai_headers(
        ...
        self.logger.info("akamai headers from env vars: {}".format(
            akamai_headers,
        ))

        if not akamai_headers:
            akamai_headers = "auto"
            self.logger.info("akamai headers: defaulting to auto")
    ...
    def load_proxies(self):
        ...
        if proxy_requirement.startswith("http://"):
            ...
            self.possible_proxies = proxy_requirement.split(',')
