...
import scrapy
from scraper.scraper.items import ProductItem, SizeItem
...
class DeBijenkorfSpider(scrapy.Spider):
    ...
    start_urls = [
        ...
        # 'https://www.debijenkorf.nl/damesmode/burberry'
        # 'https://ceres-catalog.debijenkorf.nl/catalog/product/show?productCode=8159080000&api-version=2.20'
    ]
    ...
    def parse_size(self, color_size, product):
        ...
        self.yielded_products += 1
        UpdatedProgress.set_count(round(100 * (self.yielded_products/self.total_products)))
        yield product


class UpdatedProgress:
    count = 0

    @staticmethod
    def set_count(counter):
        if UpdatedProgress.count < counter and (10 > (counter - UpdatedProgress.count) > 0):
            UpdatedProgress.count = counter

    @staticmethod
    def get_count():
        return UpdatedProgress.count
