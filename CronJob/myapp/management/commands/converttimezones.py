    ...
    def handle(self, *args, **options):
        ...
        for datetime_object in current_datetime_entries:
            ...
            if datetime_object.timezone == karachi_timezone.zone:
                self.convert_time(datetime_object, karachi_timezone)

            elif datetime_object.timezone == pytz.utc.zone:
                self.convert_time(datetime_object, pytz.utc, karachi_timezone)

    def convert_time(self, datetime_object, current_timezone, new_timezone=pytz.utc):

        new_datetime = datetime_object.now.replace(tzinfo=current_timezone)
        new_datetime = new_datetime.astimezone(new_timezone)

        DateTimeModel.objects.filter(pk=datetime_object.pk).update(
            now=new_datetime.replace(tzinfo=None),
